...
{
    //YOUR CODE GOES HERE
    vector<ItemType>preOrd;
    preOrderTraversal(root, preOrd);
    return preOrd;
...
{
    //YOUR CODE GOES HERE
    vector<ItemType>postOrd;
    postOrderTraversal(root, postOrd);
    return postOrd;
}
...
bool BST_312 <ItemType>::isItemInTree(const ItemType& item)
{
    ...
    vector<int>inTree;
    inOrderTraversal(root, inTree);

    vector<int>::iterator iter = inTree.begin();
    while (iter <= inTree.end()){
        if (*iter == item){
            return true;
        }
    }
